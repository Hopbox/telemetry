...
TUNINTERFACES=$(uci show network | grep interface | grep -v -E 'lan|self|hopcloud|loopback' | awk -F'.' '{print $2}' | awk -F'=' '{print $1}' | grep tun)
echo "got tunnels $TUNINTERFACES"
WANINTERFACES=$(uci show network | grep interface | grep -v -E 'lan|self|hopcloud|loopback|tun' | awk -F'.' '{print $2}' | awk -F'=' '{print $1}')
